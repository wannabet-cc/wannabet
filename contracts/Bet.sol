...
contract Bet {
    ...
    constructor(address _creator, address _participant, address _arbitrator, uint256 _amount, string memory _message, address _token) {
        creator = _creator;
    ...
    function acceptBet() public {
        require(msg.sender == participant, "Must be the participant to accept bet");
        require(!accepted, "Bet has already been accepted");
        require(!settled, "Bet has already been settled");
        require(amount <= IERC20(token).allowance(msg.sender, address(this)), "Must give approval to send tokens");
    ...
    function declineBet() public {
        require(msg.sender == participant, "Must be the participant to decline bet");
        require(!accepted, "Bet has already been accepted");
        require(!settled, "Bet has already been settled");
    ...
    function settleBet(address _winner) public {
		...
		require(_winner == creator || _winner == participant || _winner == 0x0000000000000000000000000000000000000000, "Winner must be a betting party");
